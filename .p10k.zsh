# Generated by Powerlevel10k configuration wizard on 2024-10-04 at 23:03 EDT.
...
() {
  ...
  function prompt_pixi() {
    local pixi_root=$PIXI_PROJECT_ROOT
    local dir=$PWD
    local label=""
    local color=6

    if [[ -z "$pixi_root" ]]; then
      # Find pixi.toml in parent directories so nested folders still show the segment.
      while [[ "$dir" != "/" ]]; do
    ...
    if [[ -n "$PIXI_PROJECT_NAME" ]]; then
      label="$PIXI_PROJECT_NAME"
      [[ -n "$PIXI_ENVIRONMENT_NAME" ]] && label+=" ($PIXI_ENVIRONMENT_NAME)"
      color=2
    elif [[ -n "$PIXI_ENVIRONMENT_NAME" ]]; then
      label="pixi ($PIXI_ENVIRONMENT_NAME)"
      color=2
    elif [[ -n "$pixi_root" ]]; then
      label="${pixi_root:t}"
      color=7
    else
      return
